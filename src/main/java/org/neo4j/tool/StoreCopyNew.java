...
public class StoreCopyNew {
    ...
    private static BufferedInputStream pbiLogIn;
    ...
    private static void copyStore(StoreCopyConfig config) throws Exception {
        try {
            ...
            BufferedOutputStream pbiLogOut = new BufferedOutputStream(new FileOutputStream(new File(config.badEntriesLogDir, "neo4j-import.log")));
            pbiLogIn = new BufferedInputStream(new FileInputStream(new File(config.badEntriesLogDir, "neo4j-import.log")));

            FormattedLogProvider logProvider = FormattedLogProvider.toOutputStream(pbiLogOut); // TODO other log output?
    ...
    private static void drainPbiLog() {
        try {
            byte[] buffer = new byte[1024];
            int len = 0;
            System.out.println();
            while ((len = pbiLogIn.read(buffer)) != -1) {
                System.out.write(buffer, 0, len);
            }
            System.out.println();
        } catch(IOException e) {
            System.err.println("Error reading neo4j-import.log:" + e.getMessage());
        }
    }
    ...
    private static void readRelationships(GraphDatabaseAPI sourceDb, BlockingQueue<InputRelationship> relQueue, long highestRelId, StoreCopyConfig config, Tokens tokens) {
        ...
        try (Transaction tx = sourceDb.beginTx(); Statement stmt = sourceDb.getDependencyResolver().resolveDependency(ThreadToStatementContextBridge.class).get()) {
            ...
            long relId = 0;
            long count = 0;
            while (running && relId <= highestRelId) {
                try {
                ...
                if (relId % BATCH_SIZE == 0) {
                    stats.count = count;
                    stats.done(running);
                    stats.printSummary("Relationships Batch " + relId / BATCH_SIZE);
                    drainPbiLog();
                }
            }
            ...
            tx.success();
            stats.done(running);
            stats.printSummary("Relationships Total");
        }
    }
    ...
    private static void readNodes(GraphDatabaseAPI sourceDb, BlockingQueue<InputNode> nodeQueue, long highestNodeId, StoreCopyConfig config, Tokens tokens) {
        ...
        try (Transaction tx = sourceDb.beginTx(); Statement stmt = sourceDb.getDependencyResolver().resolveDependency(ThreadToStatementContextBridge.class).get()) {
            ReadOperations readOps = stmt.readOperations();
            while (running && node <= highestNodeId) {
                try {
                ...
                if (count % BATCH_SIZE == 0) {
                    stats.count = count;
                    stats.done(running);
                    stats.printSummary("Nodes Batch " + node / BATCH_SIZE);
                    drainPbiLog();
                }
            }
            tx.success();
        ...
        stats.count = count;
        stats.done(running);
        stats.printSummary("Nodes Total");
    }
